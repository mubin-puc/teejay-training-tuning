/****** Object: StoredProcedure [booking].[GetPackagesByPark] Script Date: 7/2/2026 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
    SET NOCOUNT ON;  -- Added as per request (improves performance by suppressing row count messages)
    ...
    /* Precompute Bundle Percent Allocations to avoid repeated correlated subqueries */
    WITH BundleAllocation AS
    (
        SELECT 
            BundleId,
            SUM(PercentAllocation) AS TotalAllocation
        FROM product.BundleProduct
        GROUP BY BundleId
    ),
    RelatedBundleAllocation AS
    (
        SELECT 
            BundleId,
            SUM(PercentAllocation) AS TotalAllocation
    ...
    Bundles AS (
        ...
        OUTER APPLY (
            ...
            /* Precomputed allocations */
            JOIN BundleAllocation ba
                ON ba.BundleId = b.Id
            JOIN RelatedBundleAllocation rba
                ON rba.BundleId = rb.Id

            WHERE b.ParkId = @parkId
                AND b.BundleTypeId = 2 /* Party */
                
                /* Simplified date conditions (safe optimization) */
                AND ISNULL(b.StartDate, c.StartDate) <= @preferredDate
                AND ISNULL(b.EndDate, c.EndDate) >= @preferredDate
                ...
                /* Use precomputed allocations */
                AND (@usePercentAllocation = 0 
                     OR (ba.TotalAllocation = 100 AND rba.TotalAllocation = 100))
