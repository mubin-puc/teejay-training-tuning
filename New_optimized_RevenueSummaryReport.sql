...
CREATE OR ALTER PROCEDURE [report].[RevenueSummaryReport] ( -- [OPT] OR ALTER: no DROP needed to redeploy
...
BEGIN
    ...
    DROP TABLE IF EXISTS #excludedAccounts; -- [OPT]
    ...
    -- [OPT] supports the @isAggregate=1 GROUP BY and the ORDER BY in both branches
    CREATE NONCLUSTERED INDEX IX_results_Aggregate ON #results
        (TransactionDate, InvoiceNumber, PaymentId, Brand, Location, AccountId, AccountHolder, TransactionType, Provider, Source, ConfirmationCode)
        INCLUDE (ItemPrice, Discount, ZeeTax, CorpFeeTax, CorpMembershipTax, RoyaltyRevenue, ProcessingFee, Category, Product);
    ...
    SELECT DISTINCT ati.AccountId -- [OPT] was a fresh NOT EXISTS subquery in 11 blocks below; built once here
    INTO #excludedAccounts
    FROM maintenance.AccountsToIgnore ati

    CREATE UNIQUE CLUSTERED INDEX IX_excludedAccounts_AccountId ON #excludedAccounts (AccountId);
    ...
    -- [OPT] restricted to PaymentIds in this date range instead of aggregating the entire table's history every run
    ...
    WHERE afd.ApplicationFeeTypeId <> 3
      AND afd.PaymentId IN (SELECT PaymentId FROM #justiFiPayments)
    ...
    WHERE jfp.RefundedPaymentId IS NULL
        ...
        AND NOT EXISTS ( -- [OPT] was maintenance.AccountsToIgnore; now indexed temp table
            SELECT 1
            FROM #excludedAccounts ati
    ...
    -- [OPT] restricted to line items in this date range instead of scanning the entire table's history every run
    SELECT
        lis.LineItemId,
        lis.ProductName,
    ...
    WHERE EXISTS ( -- [OPT] JOIN-based EXISTS instead of a double-nested IN, clearer semi-join shape for the optimizer
        SELECT 1
        FROM payment.LineItem li
        JOIN #braintreePayments bp ON bp.InvoiceId = li.InvoiceId
        WHERE li.Id = lis.LineItemId
    )
